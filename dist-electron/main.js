...
import { sql as sql$1, eq as eq$1 } from "drizzle-orm";
...
__publicField(ConsoleLogWriter, _x, "ConsoleLogWriter");
_y = entityKind;
class DefaultLogger {
  constructor(config) {
...
__publicField(DefaultLogger, _y, "DefaultLogger");
_z = entityKind;
class NoopLogger {
  logQuery() {
  }
}
__publicField(NoopLogger, _z, "NoopLogger");
...
class SQLiteAsyncDialect extends (_rb = SQLiteDialect, _qb = entityKind, _rb) {
...
__publicField(SQLiteAsyncDialect, _qb, "SQLiteAsyncDialect");
_sb = entityKind;
...
__publicField(TypedQueryBuilder, _sb, "TypedQueryBuilder");
_tb = entityKind;
...
__publicField(SQLiteSelectBuilder, _tb, "SQLiteSelectBuilder");
class SQLiteSelectQueryBuilderBase extends (_vb = TypedQueryBuilder, _ub = entityKind, _vb) {
...
__publicField(SQLiteSelectQueryBuilderBase, _ub, "SQLiteSelectQueryBuilder");
class SQLiteSelectBase extends (_xb = SQLiteSelectQueryBuilderBase, _wb = entityKind, _xb) {
  constructor() {
    super(...arguments);
    __publicField(this, "run", (placeholderValues) => {
      return this._prepare().run(placeholderValues);
...
__publicField(SQLiteSelectBase, _wb, "SQLiteSelect");
...
__publicField(QueryBuilder, _yb, "SQLiteQueryBuilder");
_zb = entityKind;
...
__publicField(SQLiteInsertBuilder, _zb, "SQLiteInsertBuilder");
class SQLiteInsertBase extends (_Bb = QueryPromise, _Ab = entityKind, _Bb) {
...
__publicField(SQLiteInsertBase, _Ab, "SQLiteInsert");
_Cb = entityKind;
...
__publicField(SQLiteUpdateBuilder, _Cb, "SQLiteUpdateBuilder");
class SQLiteUpdateBase extends (_Eb = QueryPromise, _Db = entityKind, _Eb) {
...
__publicField(SQLiteUpdateBase, _Db, "SQLiteUpdate");
const _SQLiteCountBuilder = class _SQLiteCountBuilder extends (_Hb = SQL, _Gb = entityKind, _Fb = Symbol.toStringTag, _Hb) {
  constructor(params) {
    super(_SQLiteCountBuilder.buildEmbeddedCount(params.source, params.filters).queryChunks);
    __publicField(this, "sql");
    __publicField(this, _Fb, "SQLiteCountBuilderAsync");
    __publicField(this, "session");
    this.params = params;
...
__publicField(_SQLiteCountBuilder, _Gb, "SQLiteCountBuilderAsync");
...
__publicField(RelationalQueryBuilder, _Ib, "SQLiteAsyncRelationalQueryBuilder");
class SQLiteRelationalQuery extends (_Kb = QueryPromise, _Jb = entityKind, _Kb) {
...
__publicField(SQLiteRelationalQuery, _Jb, "SQLiteAsyncRelationalQuery");
class SQLiteSyncRelationalQuery extends (_Mb = SQLiteRelationalQuery, _Lb = entityKind, _Mb) {
  sync() {
    return this.executeRaw();
  }
}
__publicField(SQLiteSyncRelationalQuery, _Lb, "SQLiteSyncRelationalQuery");
class SQLiteRaw extends (_Ob = QueryPromise, _Nb = entityKind, _Ob) {
...
__publicField(SQLiteRaw, _Nb, "SQLiteRaw");
_Pb = entityKind;
...
__publicField(BaseSQLiteDatabase, _Pb, "BaseSQLiteDatabase");
_Qb = entityKind;
class Cache {
}
__publicField(Cache, _Qb, "Cache");
class NoopCache extends (_Sb = Cache, _Rb = entityKind, _Sb) {
  strategy() {
    return "all";
  }
  async get(_key) {
...
__publicField(NoopCache, _Rb, "NoopCache");
...
class ExecuteResultSync extends (_Ub = QueryPromise, _Tb = entityKind, _Ub) {
...
__publicField(ExecuteResultSync, _Tb, "ExecuteResultSync");
_Vb = entityKind;
...
__publicField(SQLitePreparedQuery, _Vb, "PreparedQuery");
_Wb = entityKind;
class SQLiteSession {
  constructor(dialect) {
    this.dialect = dialect;
...
__publicField(SQLiteSession, _Wb, "SQLiteSession");
class SQLiteTransaction extends (_Yb = BaseSQLiteDatabase, _Xb = entityKind, _Yb) {
...
__publicField(SQLiteTransaction, _Xb, "SQLiteTransaction");
const _LibSQLSession = class _LibSQLSession extends (__b = SQLiteSession, _Zb = entityKind, __b) {
...
__publicField(_LibSQLSession, _Zb, "LibSQLSession");
let LibSQLSession = _LibSQLSession;
const _LibSQLTransaction = class _LibSQLTransaction extends (_ac = SQLiteTransaction, _$b = entityKind, _ac) {
...
__publicField(_LibSQLTransaction, _$b, "LibSQLTransaction");
let LibSQLTransaction = _LibSQLTransaction;
class LibSQLPreparedQuery extends (_cc = SQLitePreparedQuery, _bc = entityKind, _cc) {
...
__publicField(LibSQLPreparedQuery, _bc, "LibSQLPreparedQuery");
...
class LibSQLDatabase extends (_ec = BaseSQLiteDatabase, _dc = entityKind, _ec) {
  async batch(batch) {
    return this.session.batch(batch);
  }
}
__publicField(LibSQLDatabase, _dc, "LibSQLDatabase");
function construct(client, config = {}) {
  ...
  const session = new LibSQLSession(client, dialect, schema2, { logger, cache: config.cache }, void 0);
  const db2 = new LibSQLDatabase("async", dialect, session, schema2);
  db2.$client = client;
  db2.$cache = config.cache;
  if (db2.$cache) {
    db2.$cache["invalidate"] = (_a2 = config.cache) == null ? void 0 : _a2.onMutate;
  }
  return db2;
}
...
const projects = sqliteTable("projects", {
  ...
  createdAt: integer("created_at", { mode: "timestamp" }).default(sql$1`(strftime('%s', 'now'))`),
  updatedAt: integer("updated_at", { mode: "timestamp" }).default(sql$1`(strftime('%s', 'now'))`)
});
const chapters = sqliteTable("chapters", {
  ...
  // Storing as JSON array string for simplicity
});
...
function initDb() {
  ...
  const dbPath =isDev ? path.join(process.cwd(), "dev.db") : path.join(userDataPath, "writing-app.db");
  console.log("Initializing DB at:", dbPath);
  const client = createClient({
    url: `file:${dbPath}`
  });
  db = drizzle(client, { schema });
  client.executeMultiple(CREATE_TABLES_SQL).then(() => {
...
function setupHandlers() {
  ipcMain.handle("db-save-project", async (_, { project, chapters: chapters$1, characters: characters$1 }) => {
    var _a2;
    try {
      console.log(`Saving project with ${chapters$1.length} chapters`);
      if (chapters$1.length > 0) {
        console.log(`First chapter content length: ${((_a2 = chapters$1[0].content) == null ? void 0 : _a2.length) || 0}`);
      }
      await db.insert(projects).values({
        id: project.id || "default-project",
      ...
      await db.delete(chapters).where(eq$1(chapters.projectId, projectId));
      if (chapters$1.length > 0) {
        await db.insert(chapters).values(chapters$1.map((c, index) => ({
          id: c.id,
          projectId,
          title: c.title,
          summary: c.summary,
      ...
      await db.delete(characters).where(eq$1(characters.projectId, projectId));
      if (characters$1.length > 0) {
        await db.insert(characters).values(characters$1.map((c) => ({
          id: c.id,
          projectId,
          name: c.name,
          role: c.role,
  ...
  ipcMain.handle("db-load-project", async (_, projectId = "default-project") => {
    try {
      ...
      const project = await db.query.projects.findFirst({
        where: eq$1(projects.id, projectId)
      });
      console.log("Found project:", project);
      if (!project) return null;
      const dbChapters = await db.query.chapters.findMany({
        where: eq$1(chapters.projectId, projectId),
        orderBy: (chapters2, { asc: asc2 }) => [asc2(chapters2.order)]
      });
      console.log("Found chapters:", dbChapters.length);
      const dbCharacters = await db.query.characters.findMany({
        where: eq$1(characters.projectId, projectId)
      });
